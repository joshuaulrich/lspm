...
# accept a 20% probability of a 25% drawdown
#lsp.optimize(jpt.twocoins,max.probprofit=FALSE,horizon=5,use.drawdown=TRUE,acceptable.percent=0.2,b=0.75,DEcontrol=list(NP=20))
...
# generate monthly returns for a unit size of 100 shares (excluding transaction cost and slippage)
spy.m <- OpCl(to.monthly(SPY))*100
iwm.m <- OpCl(to.monthly(IWM))*100
...
# accept a 20% probability of a 25% drawdown
lsp.optimize(jpt.real,horizon=4,max.probprofit=FALSE,use.drawdown=TRUE,acceptable.percent=0.5,b=0.75,DEcontrol=list(NP=40,CR=0,F=0.6))
