...
'deoptim' <- function(FUN, lower, upper, control = list(), ...) {  
  ...
  fn.checkBoundaries <- function(x, lower, upper) {
    f <- function(i) pmax(lower[i],pmin(upper[i],x[,i]))
    r <- apply(matrix(1:NCOL(x)), 1, f)
    return(r) 
  }
  
  d <- length(lower)
  con <- list(maximize = FALSE, VTR = -Inf, itermax = 200,
              initial = NULL, noimprove = NULL, stoptol = 0,
  ...
  if (!is.null(con$noimprove) && con$noimprove > con$itermax) {
    warning("'noimprove' > 'itermax'; ignoring 'noimprove'\n", immediate. = TRUE)
    noimprove <- NULL
  }
  if (con$NP < 1) {
  ...
  while (iter <= con$itermax & bestval >= con$VTR){
    ...
    ui <- fn.checkBoundaries(ui, lower, upper) ## check whether
    ...
    if(!con$maximize) {
	    bestval <- min(val)
            diffiter <- (tail(bestvalit,1) - bestval)
            diffiter <- ifelse(is.nan(diffiter),0,diffiter)
	    improvement <- improvement + diffiter
	  } else {
	    bestval <- max(val)
            diffiter <- (bestval - tail(bestvalit,1))
            diffiter <- ifelse(is.nan(diffiter),0,diffiter)
	    improvement <- improvement + diffiter
	  }
	  if (improvement <= con$stoptol) {
	    noimproveit <- noimproveit + 1
	  } else {
	    noimproveit <- 0
	    improvement <- 0
	  }
    ...
    if (!is.null(con$noimprove) && !is.null(con$stoptol)) {
      if (noimproveit >= con$noimprove) {
        break;
      }
    }
  }
